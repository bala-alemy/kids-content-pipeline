...
### Smart cutting (сөздің ортасынан кеспеу)

Тұрақты 30 секундпен кесу сөзді/фразаны ортасынан қиып тастауы мүмкін.
`smart_cut_enabled=true` + `cut_mode="smart_silence"` болғанда, cut point
аудиодағы ең жақын **паузаға** (тишинаға) қарай жылжытылады, сондықтан клип
сөздің ортасында бітпейді. 9:16 кадр толық сақталады (сол `fit_blur`), API
қолданылмайды.

Қалай жұмыс істейді:
- ffmpeg `silencedetect` арқылы тишина интервалдары табылады
  (`silence_threshold_db`, `min_silence_duration_seconds`);
- әр ~`clip_duration_seconds` шекарасының маңында (`cut_search_window_seconds`
  терезесінде) ең жақын тишина ортасы cut point ретінде алынады;
- cut point `min_clip_duration_seconds`..`max_clip_duration_seconds` аралығында
  ұсталады;
- сәйкес тишина табылмаса — тұрақты (fixed) шекараға fallback;
- соңғы клип 3 секундтан қысқа болса, алдыңғысына қосылады;
- әр short-қа қысқа audio fade in/out қосылады (`fade_audio_seconds`; 0 болса —
  fade жоқ; әнді бүлдірмеу үшін қысқа).

Аудио болмаса — warning шығып, бұрынғыдай fixed кесу қолданылады. Командалар
өзгермейді:
